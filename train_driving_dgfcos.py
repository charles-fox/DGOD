...
#Example run:
#python3 train_driving_dgfcos.py --exp dg --source_domains AB  --target_domains C --weights_folder AB2C --weights_file ab2c_dgfcos --reg_weights 0.5 0.5 0.5 0.05 0.0001
...
def datasetsFromArguments(source_domains, target_domains):     #IDENTICAL
...
if __name__ == '__main__':
  ...
  val_transform = albumentations.Compose([
    albumentations.Resize(height=600, width=1200, p=1.0),      ##CF: this is commented out in DGFRCNN version, why?
    albumentations.pytorch.ToTensorV2(p=1.0),
],p=1.0,bbox_params=albumentations.BboxParams(format='pascal_voc',label_fields=['class_labels'],min_area=20))



  (tr_dataset, tr_datasets, vl_dataset, test_dataset) = datasetsFromArguments(source_domains, target_domains)

  val_dataloader = torch.utils.data.DataLoader(vl_dataset, batch_size=1, shuffle=False,  collate_fn=DGFCOS.collate_fn)
  test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=1, shuffle=False,  collate_fn=DGFCOS.collate_fn)
